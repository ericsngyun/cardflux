#!/usr/bin/env python3
"""
Build FAISS index for One Piece TCG DINOv2 embeddings.

Uses IndexFlatIP for exact cosine similarity search.
For larger datasets (>100k cards), consider IVF-PQ for memory efficiency.
"""
import json
import numpy as np
import faiss
...
def main():
    print(f"\n=== Building FAISS Index for One Piece (DINOv2) ===\n")
    ...
    if not embeddings_file.exists():
        ...
        print("Run the embedder first: python services/embedder/bin/embed_onepiece_dinov2.py")
        return
    ...
    # For datasets >100k, consider IndexIVFPQ:
    # quantizer = faiss.IndexFlatIP(dimension)
    # index = faiss.IndexIVFPQ(quantizer, dimension, nlist=4096, m=8, nbits=8)
    ...
    print(f"  {index_file}")
    print(f"  {ids_file}")
    print(f"\nNote: For >100k cards, consider IVF-PQ for better memory efficiency")
